...
func (h *ssuHandler) relayPacketUDP(ctx context.Context, src net.PacketConn, ro *xrecorder.HandlerRecorderObject, log logger.Logger) error {
	...
	for {
		...
		if udpAddr, ok := targetAddr.(*net.UDPAddr); ok && (udpAddr.IP == nil || udpAddr.Port == 0) {
			log.Warnf("invalid target address from %s: target=%q", ctxAddr.ClientAddr(), targetAddr)
			continue
		}
		...
		if _, err = dstConn.WriteTo(b[:n], targetAddr); err != nil {
			...
			// probe many targets from one source port.
...
func (h *ssuHandler) packetConnForSession(ctx context.Context, src net.PacketConn, session core.UDPSession, ro *xrecorder.HandlerRecorderObject, log logger.Logger) (net.PacketConn, error) {
	...
	go func() {
		...
		for {
			...
			// where ADDR+PORT is the source of the reply. The go-shadowsocks2
			// AEAD Outbound only encrypts the plaintext as-is, so the target
			// prefix has to be prepended here before encryption.
			...
			if _, err = src.WriteTo(plaintext, addr); err != nil {
