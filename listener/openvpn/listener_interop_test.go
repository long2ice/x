...
// TestListenerInterop runs a stock `openvpn` client against the go-gost
// openvpn listener and checks a data-channel packet arrives.
func TestListenerInterop(t *testing.T) {
	dir := "/tmp/ovpn-test"
	...
	// Accept returns only after the stock client finishes the full
	// handshake through the listener. The proxied data path is covered
	// separately by TestEndToEndProxy.
	...
	case err := <-done:
		...
		t.Log("listener interop OK: stock client completed handshake through go-gost listener")
	case <-time.After(25 * time.Second):
		t.Fatal("timed out waiting for stock client")
	}
}
